import {
  type AudioToTextOptions,
  type AllOutputFormats,
  type StringOutputFormat,
  type AudioToTextFiles,
  AudioToTextJSON,
  languages,
  Proto,
} from "./types";
...
type WhisperOptions<T extends AllOutputFormats | undefined> =
  AudioToTextOptions & { output_format?: T };

// Function overload for when there are options, but the output_format is not provided
function whisper<T extends undefined>(
  audio: string,
  options: WhisperOptions<T>
): Promise<AudioToTextFiles>;
...
function whisper<T extends AllOutputFormats>(
  audio: string,
  options: WhisperOptions<T> = {}
): Promise<AudioToTextFiles | { [K in T]: Proto<K> }> {
  return new Promise((resolve, reject) => {
    const params = [audio, ...getParams(options)];
    if (options.verbose) {
      console.log("command:", params);
    }
    const whisper = spawn("mlx_whisper", params);
    ...
    whisper.on("close", (code) => {
      ...
      if (code === null || code > 0) {
        reject(
          new Error(
            `Whisper error: ${error.toString()}, CODE: ${
              code?.toString() ?? ""
            }`
          )
        );
